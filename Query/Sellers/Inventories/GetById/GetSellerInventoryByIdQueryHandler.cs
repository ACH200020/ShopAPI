...
internal class GetSellerInventoryByIdQueryHandler : IQueryHandler<GetSellerInventoryByIdQuery, InventoriesDto?>
{
    ...
    public async Task<InventoriesDto?> Handle(GetSellerInventoryByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var inventory = await connection.QueryFirstOrDefaultAsync<InventoriesDto>(sql, new { id = request.InventoryId });
